...
//  RE (Rotary Encoder) Key Function Definitions
#define RE1A KC_KP_1
#define RE1B KC_KP_2
#define RE2A KC_KP_3
#define RE2A KC_KP_4
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    ...
    if (index == 0) { //    RE1A/B
        ...
        if(IS_LAYER_ON(1)){ 
            if (clockwise) {
                tap_code16(S(KC_C));     // Shift + arrow left
                enco_counter=enco_counter+1;
                encoder_rgb_animation();
                //rgblight_step();

            } else {
                tap_code16(S(KC_D));     // Shift + arrow right (S=shift) (C=Control)
                enco_counter=enco_counter-1;
                encoder_rgb_animation();
                //rgblight_step_reverse();
            }
    ...
    if (index == 1) { //    RE2 A/B
        ...
        if(IS_LAYER_ON(1)){ 
            if (clockwise) {
                tap_code16(S(KC_KP_3));
                enco_counter=enco_counter+1;
                encoder_rgb_animation();
                //rgblight_step();

            } else {
                tap_code16(S(KC_KP_4));
                enco_counter=enco_counter-1;
                encoder_rgb_animation();
                //rgblight_step_reverse();
            }
        ...
        if(IS_LAYER_ON(3)){
            if (clockwise) {
                tap_code16(C(KC_KP_7));
                enco_counter=enco_counter+0.6;
                encoder_rgb_animation();
                //rgblight_step();

            } else {
                tap_code16(C(S(KC_KP_8)));
                enco_counter=enco_counter-0.6;
                encoder_rgb_animation();
                //rgblight_step_reverse();
            }
