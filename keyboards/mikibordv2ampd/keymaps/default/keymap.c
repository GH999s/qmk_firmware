...
bool encoder_update_user(uint8_t index, bool clockwise) {
    ...
    if (index == 0) { /* 1ST encoder */
            ...
            if(IS_LAYER_ON(1)){ 
                if (clockwise) {
                    tap_code16(S(KC_LEFT));     // Shift + arrow left
                    enco_counter=enco_counter+1;
                    encoder_rgb_animation();
                    //rgblight_step();

                } else {
                    tap_code16(S(KC_RGHT));     // Shift + arrow right (S=shift) (C=Control)
                    enco_counter=enco_counter-1;
                    encoder_rgb_animation();
                    //rgblight_step_reverse();
                }
